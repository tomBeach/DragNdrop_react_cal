class UsersController < ApplicationController
    ...
    def calendar
        ...
        rooms = Room.all
        room_names = Room.pluck(:room_name)
        sessions_count = 0

        # == instance variables
        dates = ["2018-10-02", "2018-10-03", "2018-10-04", "2018-10-05"]
        timeslots = ["09:00", "10:00", "11:00", "12:00", "13:00", "14:00", "15:00", "16:00"]

        # sql = "SELECT DISTINCT presentations.id AS session_id, presentations.room_id, presentations.session_title, presentations.session_start, rooms.room_name, abstracts.id AS abstract_id, abstracts.abs_title, abstracts.session_sequence FROM presentations INNER JOIN rooms ON rooms.id = presentations.room_id INNER JOIN abstracts ON abstracts.presentation_id = presentations.id ORDER BY presentations.session_start, rooms.room_name;"

        sql = "SELECT DISTINCT presentations.id AS session_id, presentations.room_id, presentations.session_title, presentations.session_start, rooms.id AS room_id, rooms.room_name FROM presentations INNER JOIN rooms ON rooms.id = presentations.room_id ORDER BY rooms.id, presentations.session_start;"
        sessions_collection = ActiveRecord::Base.connection.execute(sql)
        all_sessions_array = sessions_collection.to_a

        # == load session objects with abstracts assigned to session
        session_seq_array = []      # sequence array holds abstract objects of abstracts in session
        session_ids_array = []      # tracks sessions with newly added sequence arrays
        all_sessions_array.each_with_index do |session, index|
            next_session_id = session["session_id"]
            next_abstract_seq = session["session_sequence"]
            next_abstract_id = session["abstract_id"]
            next_abstract_title = session["abs_title"]

            # == add abstract data with sequence number to sessions array ([-1] indicates last element of array)
            if session_ids_array.include? next_session_id
                session_seq_array[-1]["sequences"].push({ seq:next_abstract_seq, abs:next_abstract_id, title:next_abstract_title })

            # == add session to sessions array; add abstract data to nested sequences array in sessions array
            else
                session_ids_array.push(next_session_id)
                session_seq_array.push(session)
                session_seq_array[-1]["sequences"] = []
                session_seq_array[-1]["sequences"].push({ seq:next_abstract_seq, abs:next_abstract_id, title:next_abstract_title })
            end
        end
        @calendar_data = [dates, room_names, timeslots, session_seq_array]
        # puts "@calendar_data: #{@calendar_data.inspect}"

        # == get sessions by room`
        # sql = "SELECT DISTINCT
        #     presentations.room_id,
        #     rooms.room_name,
        #     presentations.id AS session_id,
        #     presentations.session_title,
        #     presentations.session_start
        # FROM presentations
        # INNER JOIN rooms
        #     on rooms.id = presentations.room_id
        # ORDER BY presentations.session_start, rooms.room_name;"
        # sessions_collection = ActiveRecord::Base.connection.execute(sql)
        # all_sessions_array = sessions_collection.to_a
        # @calendar_data = [dates, room_names, timeslots, all_sessions_array]
